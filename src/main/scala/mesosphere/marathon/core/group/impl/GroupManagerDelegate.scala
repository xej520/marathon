...
private[group] class GroupManagerDelegate(
  ...
  /**
    ...
    * The change could take time to get deployed.//需要一定的时间来进行部署
    * //由于这个原因，我们返回部署计划，这个部署计划，marathon调度器会用到的
    * For this reason, we return the DeploymentPlan as result, which can be queried in the marathon scheduler.
    *
    * @param appId   the identifier of the application  app的标识符
    * @param fn      the application change function  app 改变的功能
    * @param version the version of the change        版本
    * @param force   if the change has to be forced.    是否强制改变
    * @return the deployment plan which will be executed.   返回要执行的部署计划
    */
  //异步返回部署计划，该计划是marathon调度器要调度的

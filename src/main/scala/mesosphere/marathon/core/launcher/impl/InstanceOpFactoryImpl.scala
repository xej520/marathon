...
class InstanceOpFactoryImpl(
  ...
  private[this] def inferForResidents(app: AppDefinition, request: InstanceOpFactory.Request): OfferMatchResult = {
    ...
    def maybeReserveAndCreateVolumes: Option[OfferMatchResult] = if (needToReserve) {
      log.info("----->InstanceOpFactoryImpl.scala<-----是否需要创建----持久卷------needToReserve={}",needToReserve)

      val configuredRoles = if (runSpec.acceptedResourceRoles.isEmpty) {
        log.info("----->InstanceOpFactoryImpl.scala<---mesos--没有设定角色的话---就给默认的---角色---- = \t" + config.defaultAcceptedResourceRolesSet)
        config.defaultAcceptedResourceRolesSet
      } else {
//        log.info("----->InstanceOpFactoryImpl.scala<-----runSpec.acceptedResourceRoles = \t" + runSpec.acceptedResourceRoles)
        runSpec.acceptedResourceRoles
      }
      // We can only reserve unreserved resources
      // 仅仅能够存储 未存储的资源，
      // Set(2,3,4).intersect(Set(3,2)) = Set(2,3)
      // 就是 求一下，交集
    ...
    maybeLaunchOnReservation
      ...
      .getOrElse {
        log.warn("------>InstanceOpFactoryImpl.scala<-----No need to reserve[存储/预留] or launch and offer request isForResidentRunSpec")

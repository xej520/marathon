...
trait RestResource {
  ...
  //Future[T]异步获取操作结果，但是，不知道什么时候去获取
  //第一种方式，不断的查看Future的完成状态
  //第二种方式，采用阻塞的方式。阻塞的方式，有两种情况方案：
    //第一个方案是，使用Await.ready, 当Future的状态为完成时返回
    //第二个方案是，使用Await.result, 直接返回Future持有的结果
